...
from .adapter_modules import InteractionBlock_global, injector_global
...
class ImageEncoderViT(nn.Module):
        ...
        **kwargs,
        ...
        if self.adapter_flag:
            ...
            self.interactions = nn.Sequential(*[
                                    InteractionBlock_global(embed_dim, num_heads=kwargs["adapter_num_heads"], 
            ...
            # mlp_vit: 2 add mlp in vit_feature
            # mlp_vit: 0 no mlp
            if kwargs["mlp_vit"] == 0:
                mlp_final = 0
            elif kwargs["mlp_vit"] == 1:
                mlp_final = 0
            elif kwargs["mlp_vit"] == 2:
                mlp_final = 1
            elif kwargs["mlp_vit"] == 3:
                mlp_final = 1
            self.final_interaction = injector_global(embed_dim, num_heads=kwargs["adapter_num_heads"], 
                                                            downsample_rate=kwargs["downsample_rate"],
                                                            with_cffn=mlp_final,
        ...
        #     self.text_convert = nn.Sequential(*[
            ...
            # self.text_convert.apply(self._init_weights)
    ...
    def forward_with_text_spa(self, x, y):
        # x [B, 3, 1024, 1024], y: [B, 1 ,768]
        #
        #
        # y = self.text_convert(y)
        ...
        for blk_index, blk in enumerate(self.blocks):
            if blk_index in self.interaction_indexes:
                ...
                x = x.view(bs, -1, dim)
                x, y = self.interactions[self.interaction_indexes.index(blk_index)](x, y)
                x = x.view(bs, H, W, dim)
            x = blk(x)
        
        # final part
        x = x.view(bs, -1, dim)
        x = self.final_interaction(x, y)
        #default
        x = x.view(bs, H, W, dim)
    ...
    def forward(self, x, y=0) -> torch.Tensor:
        if self.adapter_flag:
            
            return self.forward_with_text_spa(x, y)
